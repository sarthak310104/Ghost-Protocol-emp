  ...
  return (
    <>
      <div className="flex justify-between items-end mt-1 mb-6">
        <div>
          <div className="text-ghost-dim text-[10px] uppercase tracking-[0.13em]">
      ...
      <section className="grid grid-cols-[minmax(0,1fr)_310px] gap-3 mt-3">
          ...
          {openIncidents.map((inc) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full ${inc.severity === "critical" || inc.severity === "high" ? "bg-status-red" : inc.severity === "medium" ? "bg-status-amber" : "bg-status-green"}`} />
        ...
        <div className="bg-surface border border-border rounded-md overflow-hidden">
          ...
          {bottlenecks?.slice(0, 5).map((b, i) => (
            <div key={b.service} className="px-4 py-2.5 border-b border-[#1b1a17] last:border-0">
              <div className="flex justify-between">
                <span className="text-[#d4cec2]">
                  {String(i + 1).padStart(2, "0")} · {b.service}
                </span>
                <span className="text-[#b1aa9d]">{Math.round(b.risk_score * 100)}</span>
              </div>
              <div className="h-[3px] bg-[#1b1a17] mt-2">
                <div
                  className={`h-full ${b.risk_score > 0.75 ? "bg-status-red" : b.risk_score > 0.5 ? "bg-status-amber" : "bg-[#9f8b45]"}`}
                  style={{ width: `${Math.round(b.risk_score * 100)}%` }}
                />
              </div>
            </div>
          ))}
